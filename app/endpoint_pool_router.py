...
def fetch_compute_usage(base_url: str, *, api_key: str | None = None) -> ComputeUsage:
    ...
    routes = router.get("route_sessions", {})
    if not isinstance(routes, dict) or any(
        not isinstance(k, str) or type(v) is not int or v < 0 for k, v in routes.items()
    ):
        raise ComputeUsageSchemaError("invalid compute route session counts")
    if sum(routes.values()) > active_sessions:
        raise ComputeUsageSchemaError("route counts exceed compute active sessions")

    return ComputeUsage(
        active_sessions=active_sessions,
        max_sessions=max_sessions,
        route_sessions=routes,
    )
...
class ManagedEndpoint:
    ...
    route_active: dict[str, int] = field(default_factory=dict)
    route_connected: dict[str, int] = field(default_factory=dict)
    observed_routes: dict[str, int] = field(default_factory=dict)
    ...
    def apply_snapshot(self, snapshot: EndpointSnapshot) -> None:
        ...
        self.raw_status = snapshot.raw_status
        self.url = snapshot.url

    def invalidate_usage_sync(self, *, forget_last_success: bool) -> None:
        self.usage_synced = False
        self.usage_sync_drain_generation = None
        if forget_last_success:
    ...
    def reset_usage_after_process_stop(self, *, clear_local_sessions: bool) -> None:
        if clear_local_sessions:
            ...
            self.route_active.clear()
            self.route_connected.clear()
        self.observed_active_sessions = 0
        self.observed_routes.clear()
    ...
    def apply_usage_sync(
        ...
        *,
        ...
        self.observed_routes = dict(usage.route_sessions)
    ...
    def finish_wake(self) -> None:
        self.waking = False
        self.wake_capacity_until = None

    def finish_park(self) -> None:
        self.parking = False
        self.wake_capacity_until = None
    ...
    def finish_drain_restart(self) -> None:
        self.drain_restarting = False

    def acquire_drain(self, *, lease_id: str, now: float, lease_ttl_s: float) -> None:
        if not self.draining:
            self.drain_generation += 1
            self.draining_since = now
            self.last_drain_warning_at = None
            self.drain_lease_id = lease_id
        self.draining = True
        self.drain_expires_at = now + lease_ttl_s

    def clear_drain(self) -> None:
        self.draining = False
        self.draining_since = None
        self.drain_expires_at = None
        self.drain_lease_id = None
        self.last_drain_warning_at = None
...
class EndpointPoolRouter:
    ...
    async def start(self) -> None:
        ...
        await self._refresh_pipeline_capacity()
        if self.pipeline_capacity is not None:
            self._capacity_task = asyncio.create_task(self._capacity_loop())
    ...
    async def stop(self) -> None:
        ...
        await cancel_and_await(self._capacity_task)
        ...
        if self.pipeline_capacity is not None:
            await self.pipeline_capacity.close()

    async def _run_controller_call(
        self,
        operation: str,
        name: str,
        *,
    ...
    def _claim_slot_unlocked(
        self, *, waited_for_capacity: bool, pipeline: Optional[str] = None
        ...
        if pipeline is not None:
            endpoint.route_active[pipeline] = endpoint.route_active.get(pipeline, 0) + 1
    ...
    async def mark_connected(self, slot_id: str, *, pipeline: Optional[str] = None) -> None:
        async with self._condition:
            ...
            if pipeline is not None:
                endpoint.route_connected[pipeline] = endpoint.route_connected.get(pipeline, 0) + 1
    ...
    async def release(self, slot_id: str, *, connected: bool = False, pipeline: Optional[str] = None) -> None:
        async with self._condition:
            ...
            if pipeline is not None:
                endpoint.route_active[pipeline] = max(endpoint.route_active.get(pipeline, 0) - 1, 0)
                if connected:
                    endpoint.route_connected[pipeline] = max(endpoint.route_connected.get(pipeline, 0) - 1, 0)
            if connected:
    ...
    async def set_draining(
        ...
        *,
        ...
        async with self._condition:
            ...
            now = time.monotonic()
            if draining:
                endpoint.acquire_drain(
                    lease_id=lease_id,
                    now=now,
                    lease_ttl_s=lease_ttl_s if lease_ttl_s is not None else self.drain_lease_ttl_s,
                )
            else:
                endpoint.clear_drain()
            if draining:
    ...
    async def refresh(self) -> None:
        ...
        async def fetch_one(name: str, generation: int):
            try:
                result = await self._run_controller_call("fetch", name, fetch=True)
            except Exception as exc:
                result = exc
            return name, generation, result

        tasks = [asyncio.create_task(fetch_one(name, generation)) for name, generation in targets]
        downed_endpoints: list[str] = []
        successful_fetches = 0

        for completed in asyncio.as_completed(tasks):
            name, generation, result = await completed
            now = time.monotonic()
            async with self._condition:
                ...
                if isinstance(result, Exception):
                    endpoint.record_failure(result)
                    self._last_error = endpoint.last_error
                    log = logger.error if isinstance(result, TimeoutError) else logger.warning
                    log("Failed to fetch endpoint %s: %s", name, result)
                    continue

                successful_fetches += 1
                was_running = endpoint.running

                endpoint.apply_snapshot(result)
                if endpoint.running:
                ...
                if was_running and not endpoint.running:
                    endpoint.reset_usage_after_process_stop(clear_local_sessions=True)
                    if local_active_sessions > 0:
                        downed_endpoints.append(endpoint.name)
                elif not endpoint.running:
                    endpoint.reset_usage_after_process_stop(clear_local_sessions=False)
            ...
            for (name, url, drain_generation), result in zip(targets, results):
                ...
                if isinstance(result, Exception):
                    if isinstance(result, ComputeUnhealthyError):
                        ...
                        endpoint.invalidate_usage_sync(forget_last_success=True)
                        endpoint.record_failure(result)
                        ...
                        if stale_for is None or stale_for > self.usage_sync_stale_ttl_s:
                            ...
                            endpoint.invalidate_usage_sync(forget_last_success=False)
                        else:
                            logger.warning(
                ...
                if result.max_sessions != previous_slots:
                    logger.info(
                ...
                if result.active_sessions != previous_observed_active_sessions:
                    logger.info(
                        "Synced compute usage for %s: observed active sessions %s -> %s",
                        name,
                        previous_observed_active_sessions,
    ...
    async def _wake_endpoint(self, name: str) -> None:
        try:
            snapshot = await self._run_controller_call("wake", name)
        except Exception as exc:
            async with self._condition:
                endpoint = self._endpoints[name]
                endpoint.state_generation += 1
                endpoint.finish_wake()
                endpoint.record_failure(exc)
                self._last_error = endpoint.last_error
                self._condition.notify_all()
        ...
        async with self._condition:
            endpoint = self._endpoints[name]
            endpoint.state_generation += 1
            endpoint.apply_snapshot(snapshot)
            endpoint.parking = False
            endpoint.clear_failure()
            ...
            endpoint.reset_usage_after_fresh_process()
            self._last_error = None
    ...
    async def _park_endpoint(self, name: str) -> None:
        try:
            snapshot = await self._run_controller_call("park", name)
        except Exception as exc:
            async with self._condition:
                endpoint = self._endpoints[name]
                endpoint.state_generation += 1
                endpoint.finish_park()
                endpoint.record_failure(exc)
                self._last_error = endpoint.last_error
                self._condition.notify_all()
    ...
    async def _reconcile_loop(self) -> None:
        while True:
            ...
            try:
                ...
                logger.exception("Endpoint reconciliation failed; retrying on the next cycle")
    ...
    async def _restart_endpoint(self, name: str) -> None:
        try:
            snapshot = await self._run_controller_call("restart", name)
        except Exception as exc:
            async with self._condition:
                endpoint = self._endpoints[name]
                endpoint.state_generation += 1
                endpoint.finish_restart()
                endpoint.record_failure(exc)
                self._last_error = endpoint.last_error
                self._condition.notify_all()
    ...
    async def _capacity_loop(self):
        while True:
            await asyncio.sleep(self.pipeline_capacity.config.refresh_interval_s)
            await self._refresh_pipeline_capacity()
    ...
    def _mark_endpoints_to_wake_unlocked(
        ...
        *,
        ...
        if not force and target_count is None:
            ...
            if self._active_sessions_unlocked() == 0 and self.pipeline_capacity is None:
                return []
            target_slots = (
                self.pipeline_capacity.config.reserve_sessions if self.pipeline_capacity else self.wake_threshold_slots
            )
            if effective_free_slots >= target_slots or self._has_pending_unknown_capacity_wake_unlocked(now):
                return []
    ...
    def _should_park_endpoint_unlocked(self, endpoint: ManagedEndpoint) -> bool:
        ...
        if (
            ...
            and self._free_slots_unlocked() - endpoint.free_slots < self.pipeline_capacity.config.reserve_sessions
        ):
            return False
    ...
    async def _drain_restart_endpoint(self, name: str) -> None:
        try:
            snapshot = await self._run_controller_call("force_restart", name)
        except Exception as exc:
            async with self._condition:
                ep = self._endpoints[name]
                ep.state_generation += 1
                ep.finish_drain_restart()
                ep.record_failure(exc)
