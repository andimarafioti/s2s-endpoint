...
class WaitForInternalWSTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_wait_for_internal_realtime_http_uses_usage_endpoint(self):
        observed = {}
        ...
        with patch.object(compute_main, "_http_get_json", fake_get_json), patch.object(
            compute_main.websockets, "connect", connect
        ):
            await compute_main.wait_for_internal_realtime_http(
                compute_main.INTERNAL_WS_HOST,
                compute_main.INTERNAL_WS_BASE_PORT,
                None,
                timeout_s=0.01,
