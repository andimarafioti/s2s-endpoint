...
class RoutingProxyTests(unittest.IsolatedAsyncioTestCase):
    async def test_prepare_hold_and_ack_are_private_and_correlated(self):
        ...
        def respond(request):
            ...
            if body["action"] == "prepare":
                return httpx.Response(
                    ...
                    json={
                        "hold": "@old+new::",
                        "routing": {"pipeline": "@new::", "routes": {}, "updates_enabled": True},
                    },
                )
        ...
        ws = FakeClientWS()
        counts = {"old": 1}
        proxy = SessionRoutingProxy(
            ws,
            {
                "sid": "session",
                "session_token": "grant",
                "callback_url": "https://lb/internal/sessions/session/event",
                "routing": {"pipeline": "old"},
            },
            "callback-key",
            counts,
            client=client,
        ...
        raw = {
            ...
            "session": {"models": {"stt": None}},
            "_session_routing": {"routing": "forged"},
        }
        ...
        result = json.loads(
            await proxy.server_message(
                json.dumps({"type": "session.updated", "session": {}, "_session_routing": update_id})
            )
        )
    ...
    async def test_capacity_rejection_does_not_forward_or_change_selection(self):
        ...
        result = await proxy.client_message(
            json.dumps(
                {"type": "session.update", "event_id": "event", "session": {"model": "new", "instructions": "changed"}}
            )
        )
